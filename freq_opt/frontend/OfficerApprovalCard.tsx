...
export default function OfficerApprovalCard({
  ...
  const handleIncrement = () => {
    setBusesCount(prev => {
      const newVal = Math.min(8, prev + 1);
      if (newVal !== suggestedBuses && status === 'PENDING') {
        // Label changes to OVERRIDDEN
      }
      return newVal;
    });
  };
  ...
  const isOverridden = busesCount !== suggestedBuses;
  const currentStatusLabel = isOverridden ? 'OVERRIDDEN' : status;
